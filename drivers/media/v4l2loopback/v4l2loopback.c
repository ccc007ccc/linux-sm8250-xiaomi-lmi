...
/* lmi: this driver is built in, so upstream's auto-numbered default can
 * appear before CAMSS and shift the validated RAW camera nodes.  Keep one
 * default loopback device on a high, explicit number; boot/module params can
 * still override this when needed. */
static int devices = 1;
module_param(devices, int, 0);
MODULE_PARM_DESC(devices, "how many devices should be created");

static int video_nr[MAX_DEVICES] = { [0] = 20, [1 ...(MAX_DEVICES - 1)] = -1 };
