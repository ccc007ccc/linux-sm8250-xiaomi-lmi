					...
					unsigned int *val)
{
	...
	if (IS_ERR(chip->usb_in_i_chan)) {
		dev_err(chip->dev, "Failed to get usbin_i, err = %li", PTR_ERR(chip->usb_in_i_chan));
		return PTR_ERR(chip->usb_in_i_chan);
	}

	rc = iio_read_channel_processed(chip->usb_in_i_chan, val);
