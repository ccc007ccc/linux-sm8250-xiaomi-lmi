...
| 摄像头 | 部分支持（bring-up） | 第一阶段只接入后置超广角 OV13B10；CAMSS/CCI power-domain 问题已通过内置 SM8250 CAMCC 修复，OV13B10 已在 `&cci0_i2c1` 地址 `0x10` probe 成功，当前已通过标准 media/V4L2 ioctl 路线跑通 `/dev/video3` raw Bayer `pgAA` stream，驱动暴露的 6 个 4-lane 模式（4208x3120/4160x3120/4160x2340/2104x1560/2080x1170/1364x768）均已实机 raw capture 验证；frame interval、crop selection、orientation/rotation、media route、video querycap、控制项扩展元数据（含 `Unit Cell Size` 1120nm x 1120nm）、video frame-size probing 和按 OV13B10 RAW10 media-bus code 收敛后的 format enumeration 等标准发现信息可查询。`lmi-camera-web-preview.py` 是不改 rootfs 的 raw-camera 支持工具，提供稳定 `lmi.raw-camera.discovery.v1` 发现 JSON、彩色 PNG/HTTP multipart 预览、`/snapshot`/`/raw`/`/metadata` Web 导出端点、`/capabilities` HTTP 能力发现、raw frame/metadata sidecar 导出、percentile AE、过滤 AWB 和 preview CCM/gamma 软件处理；同时已接入用户态 C software-ISP（`lmi-isp`）+ v4l2loopback/UVC gadget，可把 RAW 转成独立标准 YUYV/NV12 节点，实测 2080x1170→1280x720 YUYV 约 29fps；低延迟未压缩 UVC 档为 640x480@15 + 2 buffers，DirectShow/ffmpeg 主机取流约 13fps；MJPEG UVC 压缩输出已实机验证 2080x1170 RAW 输入到 640x480@30、960x540@30 和 1280x720@30，Windows MediaCapture 可枚举 MJPG、连续拍照和短 MP4 录制；720p 开机档已提升到 quality 78、`max-frame=1048576`，并用 Bayer-aware box averaging 改善单 Bayer 点采样导致的主观细节偏低，设备侧实测约 27.9–29.3fps；Windows PnP 在断开/重连恢复后仍显示 `UVC Camera` OK，并可通过 digital/software gain 上限降低噪点；helper 已加入 `--isp-uvc-demand-start`，可让 UVC gadget 先保持可枚举，只有 host `STREAMON` 时才启动 RAW/CAMSS/`lmi-isp`，且已验证开机默认 UVC demand-start：Windows/usbipd 枚举 `1d6b:0102 UVC Camera`，idle 时无独立 `lmi-isp`，Windows MediaCapture 打开后才触发 `STREAMON` 并启动 `/run/lmi-camera/lmi-isp`，关闭后 `STREAMOFF` 并回到只剩 manager+feeder 的枚举状态。 | 当前**内核**边界仍是 raw RDI/Bayer，`/dev/video3` 输出 `pgAA`，SM8250 mainline CAMSS 仍无完整 ISP/YUV/RGB 输出；Android 运行时 trace 已确认 OV13B10 在厂商 HAL 下会启用 DISP/FD/stats/PDAF/LCR/RDI 等 VFE480 BUS/comp-group 资源，但这仍只是下游运行时证据，不等同于主线已输出 YUV/RGB。用户态 software-ISP/loopback/UVC 是独立标准节点，不把 raw node 伪装成 YUV/RGB；v4l2loopback 默认节点为 `/dev/video20`，已用标准 V4L2 程序对 capture 侧完成 `QUERYCAP`/`ENUM_FMT`/`S_FMT`/`read()` 多帧验证，PipeWire/browser `getUserMedia` 仍待下一阶段验证；当前性能上限按 2080x1170 输入评估，不继续追更高输入分辨率；USB-HS 未压缩 UVC 现实优先档位仍是 640x480@15（720p@10 可用但更容易累积主机端延迟）；MJPEG UVC 目前按 Windows MediaCapture still/record 验证，PowerShell WinRT frame-reader 脚本拿不到 `MediaFrameSource` 不作为功能失败；开机默认 UVC demand-start 由 initramfs 在 `/run` tmpfs 生成 transient systemd unit 和 `/run/lmi-camera` 工具/运行时文件（含 FIFO/event FIFO/ready file），并用 `/run` 临时 mask 旧 rootfs 调试 USB gadget、让 UVC unit `Conflicts=`/`Before=` 该旧服务，避免同一 UDC 被抢占；只有 UVC unit staging 成功才解绑早期 ACM，失败则保留维护通道；不向普通 rootfs 落盘安装；可用 `lmi.uvc=0`/`lmi.no_uvc` 禁用；H.264 UVC 仍只是 Venus-gated scaffold，不作为可用功能宣传；软件 auto-tone 只是用户态 software-ISP 输出增益，不等于内核真 ISP/YUV；`/dev/video3` 的 frame-size enum 是 CAMSS 通用连续范围，离散模式和帧间隔应以 OV13B10 subdev 为准；前摄升降机构和其他后摄暂不启用。 |
