...
- 在内核 ISP 配方被证不可提取后，相机转向**用户态 software-ISP → 标准相机节点**并已实机跑通：vendored v4l2loopback 0.15.3（in-tree `=y`，当前 lmi-local 默认补丁让 built-in loopback 建在 `/dev/video20`，避免挤占 CAMSS RAW 节点编号）；`lmi-camera-web-preview.py --isp-engine c` 默认 spawn `/tmp/lmi-isp`，把 OV13B10 RAW(`pgAA`/`bayer10p`) 做 10-bit 解包、AWB、bilinear demosaic、gamma 2.2 tone curve、YUYV/NV12 pack 后写入 loopback 或 UVC FIFO。纯 Python 640x480 只有约 2–5fps，但 C engine 已验证 2080x1170 RAW → 1280x720 YUYV **29.4fps**（约 22ms/frame）；UVC gadget + `lmi-uvc-gadget` feeder 已让 Windows 免驱识别 `UVC Camera` 并由 MediaCapture 实际保存 1280x720 JPEG。USB-HS 下未压缩 UVC 以 720p@10 或 640x480@15 为现实档位；当前性能上限按 2080x1170 输入评估，不继续追更高输入分辨率。本机无桌面 `/dev/video20` 标准 V4L2 程序调用已通过 `QUERYCAP`/`ENUM_FMT`/`S_FMT`/`read()` 5 帧验证；PipeWire/browser `getUserMedia` 仍待下一阶段验证。RAW 仍真实、`/dev/video3` 不宣告假格式、`vfe480_yc_pp_chain_configured()` 仍关闭、`/dev/video6` 仍 `BA10` 诊断。详见 [CAMERA_BRINGUP.md](CAMERA_BRINGUP.md) 第 31 条。
