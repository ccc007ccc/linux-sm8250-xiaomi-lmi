...
### M73 image40 DONE 后 HELLO command-mode 覆盖诊断

含义：M73 在 `lmi-sahara-loader.py` 中新增默认关闭的 `--done-hello-mode-image` / `--done-hello-mode` 诊断开关，并在 rootfs `lmi-sahara-test` 中暴露 `DONE_HELLO_MODE_IMAGE` / `DONE_HELLO_MODE`。该开关只在指定 image 的 `END_OF_IMAGE` 后发送 `DONE`，随后收到下一次 HELLO 时覆盖这一包 HELLO_RESP 的 mode，用来单独测试 image40 DONE 后的新 HELLO 是否需要 command mode，而不影响 image34、image40 传输前面的 HELLO_RESP。

当前影响：干净 reboot 后按 M71 成功 cadence 复测，并且只额外叠加 `--done-hello-mode-image 40 --done-hello-mode 3`。本轮稳定复现 image34 两段、`END_OF_IMAGE image=34 status=0`、`DONE_RESP status=0`、第二轮 HELLO、image40 offset 0/52、52/96、4096/6712、12288/1220 和 `END_OF_IMAGE image=40 status=0`。image40 `DONE` 后收到新的 mode0 HELLO，loader 定向写入 `HELLO_RESP ... mode=3 override=1`；dmesg 同步确认该包被解析为 `SAHARA HELLO_RESP decoded ... mode=3`，并且 er1 上有 `SAHARA UL completion status 0 bytes 48`。随后同 session 8 秒 idle，后续三轮 session 均为 packets=0，最终只因 `EMPTY_LIMIT=3` 停止；只读状态仍是 `AP2MDM_STATUS=1 AP2MDM_ERRFATAL=0 MDM2AP_STATUS=0 MDM2AP_ERRFATAL=0 cached/reg=SECONDARY BOOTLOADER/M0 pm_state=0x4`，未出现 `SYS_ERROR` 或 `INVALID_EE`。
...
当前影响：项目内 `lmi/scripts/lmi-sahara-loader.py` 与 rootfs `/usr/local/bin/lmi-sahara-loader` 已补齐被动 command-exec 诊断：默认 `--cmd-exec 9`，并暴露 `--max-command-response` 和 `--command-timeout`；rootfs `/usr/local/sbin/lmi-sahara-test` 同步暴露 `CMD_EXEC`、`MAX_COMMAND_RESPONSE`、`COMMAND_TIMEOUT` 环境变量。loader 只在真实 `CMD_READY` 后发送 `CMD_EXEC` / `CMD_EXEC_DATA`，并只记录 raw response 的长度、SHA256 和前 32 字节，不把 response 写入 modem/NV/分区，也不把 firmware 或 response blob 纳入 git。该改动已通过 Python 和 shell 语法检查。M78 实机干净 reboot 后按 M71/M75 cadence 加 `--unsafe-done-restart --ks-pending-timeout 20` 复测，未出现 `CMD_READY`，因此没有发送 `CMD_EXEC`；但 image40 offset 0/52、52/96、4096/6712、12288/1220、`END_OF_IMAGE image=40 status=0`、`DONE_RESP image_tx_pending=0` 和 post-DONE HELLO/HELLO_RESP 之后，首次收到真实 image41/devcfg `READ_DATA 3 20 41 0 52`。loader 从 modem FAT `image/sdx55m/devcfg.mbn` 被动返回 52 字节，内核确认 `SAHARA UL completion status 0 bytes 52`；随后收到一个 20 字节全零包，后续三个 20 秒 session 为空，字符设备仍存在，最终只读状态仍为 `AP2MDM_STATUS=1 AP2MDM_ERRFATAL=0 MDM2AP_STATUS=0 MDM2AP_ERRFATAL=0 cached/reg=SECONDARY BOOTLOADER/M0 pm_state=0x4`。
...
### M91/M92 SAHARA grouped transfer 与 restart context 诊断

含义：M91/M92 继续围绕 M90 的 SAHARA restart 路径收敛。M92 把用户态每个 `READ_DATA` 响应改为一次 `write()`，内核侧再按 MTU 拆成多个 TRE：中间 TRE 使用 `MHI_CHAIN`，最后一个 TRE 使用 `MHI_EOT`，用于验证大镜像 payload 是否因 userspace 多次写入而和 Android downstream UCI 的单次 transfer 语义不同。

当前影响：M92 boot-only 镜像只刷 `boot` 后可启动，但按 M90-like 参数 `restart_after_ul_delay_ms=200`、`restart_track_read_data_len=4096`、`restart_suppress_read_data_len=131072`、`--continuous --active-read-drain` 复测时没有进入预期的大包 grouped 路径：image34/mdmddr 完整、`DONE_RESP image_tx_pending=0`、新 HELLO/HELLO_RESP 后，只推进到 image40/APDP offset 0/52 和 52/96，随后 200 秒 idle timeout。为隔离用户态 grouped 改动，在同一 M92 内核上运行 HEAD/M90 loader 也停在 image40 offset 0/52 后 idle。该对照说明 M92 回退不是单纯由 userspace grouping 引起，更可疑的是当前诊断用 `mhi_unprepare_from_transfer()` / `mhi_prepare_for_transfer()` 反复 full channel restart 后，host ring、channel context WP、doorbell value 或 stale event 状态发生不同步；M91/M92 也再次证明 full restart cadence 存在非确定性，可能从 M90 的 image33/HYP EOI 最大进展回退到 image40 前段。
